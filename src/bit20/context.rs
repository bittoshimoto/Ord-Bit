...
use crate::bit20::operation::{Action, deserialize_bit20_operation, InscriptionOp, Operation};
use crate::bit20::transfer::Transfer;
use crate::bit20::TransferInfo;
...
impl Message {
  pub(crate) fn resolve<'a, 'db, 'tx>(
    ...
    let bit20_operation = match op.action {
      Action::New { inscription: _ } if sat_in_outputs => {
        match deserialize_bit20_operation(
          new_inscriptions
          ...
          Ok(bit20_operation) => bit20_operation,
          _ => return Ok(None),
        }
      }
      // Transfered inscription operation.
      // Attempt to retrieve the `InscribeTransfer` Inscription information from the data store of BIT20.
      Action::Transfer => {
        match get_inscribe_transfer_inscription(bit20_inscribe_transfer, op.inscription_id) {
