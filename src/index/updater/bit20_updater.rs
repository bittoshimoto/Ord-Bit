...
use crate::bit20::{Balance, BlockContext, Deploy, DeployEvent, BIT20Error, Event, InscripbeTransferEvent, max_script_tick_key, Message, min_script_tick_key, Mint, MintEvent, Num, script_tick_key, Tick, TokenInfo, Transfer, TransferableLog, TransferEvent, TransferInfo};
use crate::bit20::errors::Error::LedgerError;
use crate::bit20::operation::{InscriptionOp, Operation};
use crate::bit20::script_key::ScriptKey;
...
pub(super) struct Bit20Updater<'a, 'db, 'tx> {
    bit20_token_info: &'a mut Table<'db, 'tx, &'static str, &'static [u8]>,
    bit20_token_balance: &'a mut Table<'db, 'tx, &'static str, &'static [u8]>,
    bit20_inscribe_transfer: &'a mut Table<'db, 'tx, &'static [u8; 36], &'static [u8]>,
    bit20_transferable_log: &'a mut Table<'db, 'tx, &'static str, &'static [u8]>,
...
impl<'a, 'db, 'tx> Bit20Updater<'a, 'db, 'tx> {
    pub(super) fn new(
        bit20_token_info: &'a mut Table<'db, 'tx, &'static str, &'static [u8]>,
        bit20_token_balance: &'a mut Table<'db, 'tx, &'static str, &'static [u8]>,
        bit20_inscribe_transfer: &'a mut Table<'db, 'tx, &'static [u8; 36], &'static [u8]>,
        ...
        Ok(Self {
            bit20_token_info,
            bit20_token_balance,
            bit20_inscribe_transfer,
            bit20_transferable_log,
    ...
    pub(crate) fn index_block(
        ...
        log::info!(
      "BIT20 Updater indexed block {} with {} messages in {} ms",
      context.blockheight,
      messages_size,
    ...
    pub fn resolve_message(
        ...
        for input in &tx.input {
            ...
            while let Some(operation) = operation_iter.peek() {
                ...
                // Parse BIT20 message through inscription operation.
                if let Some(msg) =
                    Message::resolve(&mut self.bit20_inscribe_transfer, &new_inscriptions, operation)?
                {
                    messages.push(msg);
                    continue;
                }
    ...
    fn process_deploy(
        ...
        deploy: Deploy,
    ) -> Result<Event, errors::Error<BIT20Error>> {
        // ignore inscribe inscription to coinbase.
        let to_script_key = msg.to.clone().ok_or(BIT20Error::InscribeToCoinbase)?;
        ...
        {
            return Err(errors::Error::BIT20Error(BIT20Error::DuplicateTick(
        ...
        if supply.sign() == Sign::NoSign
            || supply > bit20::params::MAXIMUM_SUPPLY.to_owned()
        {
            return Err(errors::Error::BIT20Error(BIT20Error::InvalidSupply(
                supply.to_string(),
            )));
        }
        ...
        if limit.sign() == Sign::NoSign
            || limit > bit20::params::MAXIMUM_SUPPLY.to_owned()
        {
            return Err(errors::Error::BIT20Error(BIT20Error::MintLimitOutOfRange(
    ...
    fn process_mint(
        ...
        let token_info = Self::get_token_info(self, &tick)
            .map_err(|e| LedgerError(e))?
            .ok_or(BIT20Error::TickNotFound(tick.to_string()))?;

        let mut amt = Num::from_str(&mint.amount)?;

        if amt.sign() == Sign::NoSign {
            return Err(errors::Error::BIT20Error(BIT20Error::InvalidZeroAmount));
        }
        if amt > Into::<Num>::into(token_info.limit_per_mint) {
            return Err(errors::Error::BIT20Error(BIT20Error::AmountExceedLimit(
                amt.to_string(),
            )));
        }
        ...
        if minted >= supply {
            return Err(errors::Error::BIT20Error(BIT20Error::TickMinted(
    ...
    fn process_inscribe_transfer(
        ...
        if amt.sign() == Sign::NoSign || amt > Into::<Num>::into(token_info.supply) {
            return Err(errors::Error::BIT20Error(BIT20Error::AmountOverflow(
                amt.to_string(),
            )));
        }
        ...
        if available < amt {
            return Err(errors::Error::BIT20Error(BIT20Error::InsufficientBalance(
                available.to_string(),
                amt.to_string(),
            )));
        }
    ...
    fn process_transfer(
        ...
        let transferable = Self::get_transferable_by_id(self, &msg.from, &msg.inscription_id)
            .map_err(|e| LedgerError(e))?
            .ok_or(BIT20Error::TransferableNotFound(msg.inscription_id))?;

        let amt = Into::<Num>::into(transferable.amount);

        if transferable.owner != msg.from {
            return Err(errors::Error::BIT20Error(BIT20Error::TransferableOwnerNotMatch(
                msg.inscription_id,
            )));
        }
    ...
    fn insert_transferable(
        ...
        self.bit20_transferable_log.insert(
    ...
    fn insert_inscribe_transfer_inscription(
        ...
        self.bit20_inscribe_transfer.insert(
            &value,
    ...
    fn update_token_balance(
        ...
        self.bit20_token_balance.insert(
