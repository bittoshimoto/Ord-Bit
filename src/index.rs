...
define_table! { BIT20_BALANCES, &str, &[u8] }
define_table! { BIT20_TOKEN, &str, &[u8] }
define_table! { BIT20_INSCRIBE_TRANSFER, &[u8; 36], &[u8] }
define_table! { BIT20_TRANSFERABLELOG, &str, &[u8] }
...
impl Index {
    ...
    let database = match unsafe { Database::builder().open(&path) } {
      Ok(database) => {
        {
          ...
          index_transactions = statistics
            ...
            .value()
            != 0;
          index_bit20 = statistics
            .get(&Statistic::IndexBit20.key())?
            .unwrap()
            .value()
            != 0;
        }
        ...
        if error.kind() == io::ErrorKind::NotFound =>
      {
...

        {
          ...
          index_bit20 = options.index_dunes();
          index_dunes = options.index_dunes();
          index_sats = options.index_sats;
          index_transactions = options.index_transactions;

          statistics.insert(&Statistic::IndexBit20.key(), &u64::from(index_bit20))?;
  ...
  pub(crate) fn get_bit20_balances(&self, script_key: &ScriptKey) -> Result<Vec<Balance>> {
    if self.block_count().unwrap() >= self.first_inscription_height {
      let rtx = self.database.begin_read()?;

      let bit20_token_balance = rtx.open_table(BIT20_BALANCES)?;
  ...
  pub(crate) fn get_bit20_balance(
    &self,
  ...
  pub(crate) fn get_bit20_token_info(&self, tick: &Tick) -> Result<Option<TokenInfo>> {
    if self.block_count().unwrap() >= self.first_inscription_height {
      let rtx = self.database.begin_read()?;

      let bit20_token_info = rtx.open_table(BIT20_TOKEN)?;
      return Ok(
        bit20_token_info
  ...
  pub(crate) fn get_bit20_tokens_info(&self) -> Result<Vec<TokenInfo>> {
    if self.block_count().unwrap() >= self.first_inscription_height {
      let rtx = self.database.begin_read()?;
