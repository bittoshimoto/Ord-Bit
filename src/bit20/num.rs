...
impl Num {
    pub fn checked_add(&self, other: &Num) -> Result<Self, BIT20Error> {
        Ok(Self(self.0.clone() + &other.0))
    }

    pub fn checked_sub(&self, other: &Num) -> Result<Self, BIT20Error> {
        if self.0 < other.0 {
            return Err(BIT20Error::Overflow {
    ...
    pub fn checked_to_u128(&self) -> Result<u128, BIT20Error> {
        if !self.0.is_integer() {
            return Err(BIT20Error::InvalidInteger(self.clone().to_string()));
        }
        self
            .0
            .to_bigint()
            .ok_or(BIT20Error::InternalError(format!(
                "convert {} to bigint failed",
                self.0
            )))?
            .to_u128()
            .ok_or(BIT20Error::Overflow {
...
impl FromStr for Num {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        if s.starts_with('.') || s.ends_with('.') || s.find(['e', 'E', '+', '-']).is_some() {
            return Err(BIT20Error::InvalidNum(s.to_string()));
        }
        let num = BigDecimal::from_str(s).map_err(|_| BIT20Error::InvalidNum(s.to_string()))?;
